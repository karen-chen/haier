  ...
  <div class="navigationstrat">
    ...
    <div class="others nearby" v-for="item in navigationList">
      <div class="main" v-show="item.navigationStatus == 1">
        ...
        <div class="startBtn" @click="navigationstratBtn(item)">开始导航</div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getBaseConfig() {
      //分享链接
      config.wxShareData.link = config.cdnUrl + '/index.html?actSid=' + userData.actSid + '#/navigationstart';
      //文章标题
      document.title = '会场导航';
