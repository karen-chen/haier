  ...
  <div class="activity_main m20" v-show="showMain">
    ...
    <div class="el-row p0" v-show="obj.showPic">
      ...
      <div class="el-col el-col-22">
        <div class="el-row p0">
          <div class="el-col el-col-5" v-for="pic in obj.bannerItemList" @click="openpic(pic,obj.bannerItemList)">
            <div class="shop_pic"><img :src="imageServerUrl + pic.imgUrl"></div>
          </div>
...
/*    _this.shopId = _this.$route.params.shoplist;*/
    ...
    if (this.$route.params.shoplist) {
      localStorage.setItem('shopCode', this.$route.params.shoplist)
   ...
   /* var shareId = this.shopCode*/
    ...
    methods:{
      getShopDetail(){
        console.log(454545454,this.shopCode)
        var _this = this
        $.ajax({
          type: "GET",
          ...
          dataType: "jsonp",
          data: {
            shopId:this.shopCode
          },
          success: function(data) {
            if (data.ret == 0) {
              _this.obj=data.obj;
              _this.code = data.obj.code;
              localStorage.setItem('shopcode',_this.code)
              ...
              if(_this.obj.bannerItemList  == [] || _this.obj.bannerItemList.length<=0){
                _this.obj.showPic = false
              }else {
                _this.obj.showPic = true
              }
              ...
              config.wxShareData.link = config.cdnUrl + '/index.html?shopCode=' + _this.shopCode + '#/shopdetail';
              wxShare()
              _this.getCommentTotal()
      ...
      getCommentTotal(){
        var _this = this
        _this.code = localStorage.getItem('shopcode')
        $.ajax({
          type: "GET",
      ...
      openpic(pic,_bannerItemList){
        var _this = this
        var _urls = [];
        _bannerItemList.forEach(function(_pic){
          var piclist = _this.imageServerUrl+_pic.imgUrl
          _urls.push(piclist)
        })
        wx.ready(function(){
          wx.previewImage({
            current: _this.imageServerUrl+pic.imgUrl, // 当前显示图片的http链接
            urls: _urls // 需要预览的图片http链接列表
          });
        })
      },
