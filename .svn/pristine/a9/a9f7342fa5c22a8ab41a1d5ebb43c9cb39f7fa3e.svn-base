<template>
  <div class="content">
    ...
    <div class="popBox" v-show="popShoplist">
      <div class="shopPop">
        <div class="pop_information">
          <div class="titlePop">添加用户购买的商品信息</div>
          <div class="itemlist">
            <ul class="selectBar">
              <li>
                <select v-model="selectItem" @change="changeItem()" @focus="textCenter1" @keydown="downItem()" id="select">
                  <!-- listcat @change="areaChange('province')"-->
                  <option v-for="(list,index) in listcat" :key="index" :value="list">{{list.cat_title}}</option>
                </select>
                <div class="selectBtn"></div>
                <div class="selectText" v-show="selectText">请选择商品类型</div>
              </li>
              <li>
            ...
            <ul class="shopItem">
              ...
              <div @click="addMore" v-show="addMorelist" class="addMore">点击加载更多</div>
            </ul>
          </div>
        </div>
        <div class="off_btn" @click="offBtn">&times;</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      selectText: true,
      catTitle:{
        cat_title:"请选择商品类型",selected:true}
    }
  },
  ...
  methods: {
    ...
    list_cat() {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          self.listcat.unshift(self.catTitle)
          if (data.data.length > 0) {
            data.data.forEach(el => {
              self.listcat.push(el);
              el.selected = true
            })
            console.log(44444,self.listcat)
          } else {
            self.listcat = [];
          }
        }
    ...
    downItem() {
      ...
      console.log(6666,this.selectItem.sid)
      this.cat_sid = this.selectItem.sid;
      var self = this;
      this.pageNumber = 1;
      self.shopItem = [];
  ...
  .shopPop {
    ...
    .itemlist {
      ...
      .addMore {
        ...
        height: pTR(42);
        border-radius: pTR(5);
        border: pTR(2) solid #f66051;
        color: #f66051;
        font-size: pTR(24);
        text-align: center;
        line-height: pTR(42);
        margin: pTR(30) auto pTR(20);
      }
    }
  }
}
