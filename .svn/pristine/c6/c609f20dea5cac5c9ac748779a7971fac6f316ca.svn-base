    ...
    <ul class="bottom">
      <li class="all">
        <div class="quanitem" v-for="item in allVouchers">
            <a class="item1" @click="detailBtn(item)">
          ...
          <div class="item2">
            ...
            <p @click="fullSuccess(item)" v-show="((item.orderStatus==2 ||item.orderStatus==3 || item.orderStatus==8) && item.customerInfoFlag==0) && item.canCustomerInfo==1"><span>补充信息</span><i></i></p>
      ...
      <li class="null" v-show="nullBox && nullBox1">
  ...
  methods: {
    ...
    detailBtn(item) {
      if (item.orderStatus == 7) {
       ...
       localStorage.setItem('cardOrderId', item.sid);
            this.$router.push({
        name: 'Offsetquan',
        // params: {
        //   sid: item.sid,
