...
export default {
  ...
  created() {
    ...
    if ($.inArray(window.location.hash.substr(2), ['flow','runpc','flowpage', 'purchase-process/flow', 'loginadmin', 'salesmen/loginadmin', 'binding', 'salesmen/binding']) >= 0 || wxChankeAuthorize()) {
      //判断页面路由跳转
      this.toPage(data);
    } else {
      return;
  ...
  methods: {
    toPage(page) {
      switch (page) {
        ...
        case 'runbox': //salesmen/binding
          this.$router.push('/runphone')
          break;


        // case 'runpcnow': //salesmen/binding
        //   this.$router.push('/runpc')
        //   break;
        default:
