  ...
  <div class="seach_body list_body">
    ...
    <div class="el-row row_p17">
      ...
      <div class="el-col el-col-14 inputbox">
        <input placeholder="输入门店名或地址" class="el-input" v-model="seachValue">
      ...
      <div class="row_p0 el-row" v-show="showBanner">
        <div class="swiper-container">
          <div class="swiper-wrapper">
            <div class="swiper-slide center-swiper" v-for="banner in list"  ><img :src="imageServerUrl + banner.imgUrl" :data-src="banner.linkUrl"></div>
          </div>
  ...
  methods: {
    ...
    areaChange(type) {
      switch (type) {
        ...
        {
          if (this.result.city) {
            //把json字符串转换成 json对象   以便于之后做switch循环
            let province = JSON.parse(this.result.province)
            let city = JSON.parse(this.result.city)
            this.country = AreaData.dists_data[city.value]
            switch(province.text) {
              case "北京市":
              case "天津市":
              case "上海市":
              case "重庆市":
                this.result.city = this.result.province
                break;
            }
          }
          break;
    ...
    listShop(item,index) {
      ...
      if (this.choose_index >= 0) {
        this.$router.push({
          name: 'Activity',
          params: {
            shoplist: this.rows[this.choose_index].code
          }
        })
      }
    },
