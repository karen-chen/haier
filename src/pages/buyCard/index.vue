<template>
  <div class="newpage">
    ...
    <div class="mainBox">
      <div class="chooseItem" v-show="funcDisplayFlag">
        <a class="left" v-show="shakeDisplayFlag" :href="shakeUrl"><i></i>活动摇奖</a>
        <div class="right" v-show="consultDisplayFlag" @click="zixun"><i></i>活动咨询</div>
        <span class="line" v-show="consultDisplayFlag && shakeDisplayFlag"></span>
      </div>
      <div class="detailBox">
        <div class="detailTtitle">
          <div class="left"></div>
          <div class="right">活动介绍 </div>
        </div>
        <div class="detailList">
          <!--           <div class="detailItem">
            <div class="detailtime">活动:</div>
        ...
        <div class="moredetail" @click="moreClick">活动详情<span></span></div>
      </div>
      <div class="soundBox" v-show="soundBox">
        ...
        <div class="soundList">
          ...
          <div class="soundtxt">{{ad_duration}}'</div>
      ...
      <div class="goline" v-show="userDataactSid == '21f4646411694dd093f13dcf0374234f'"><a class="gobtn" href="http://1251097942.cdn.myqcloud.com/1251097942/haier/online/index.html?actSid=eabc999059974724bc878998f0673c8b#/qshake"></a></div>
      <div class="shopBox">
        <div class="shoptitle">
          <div class="left"></div>
    ...
    <!--弹出销售二维码-->
<!--     <div class="popmask" v-show="saleBox">
      <div class="saleMain">
        <h2>请长按二维码</h2>
        <p>添加海尔销售员002咨询</p>
        <img class="saleMa" :src="consultImg">
        <i class="sclose" @click="maclosebtn"></i>
      </div>
    </div> -->
...
export default {
  ...
  data() {
    return {
      ...
      soundBox: false,
      ad_duration: '',
      funcDisplayFlag: false,
      shakeDisplayFlag: false,
      consultDisplayFlag: false,
      shakeUrl: '',
      consultImg: '',
      saleBox: false
    }
  ...
  methods: {
    ...
    getBaseConfig() {
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data.ret === "0") {
            if (data.funcDisplayFlag == 1) {
              self.funcDisplayFlag = true
            }
            if (data.shakeDisplayFlag == 1) {
              self.shakeDisplayFlag = true
            }
            if (data.consultDisplayFlag == 1) {
              self.consultDisplayFlag = true
            }
            if (data.shakeDisplayFlag == 0 && data.consultDisplayFlag == 0) {
              self.funcDisplayFlag = false
            }
            self.shakeUrl = data.shakeUrl;
            self.consultImg = config.imageServerUrl + data.consultImg;

            self.activityAddress = data.activityAddress;
            self.activityTime = data.activityTime;
            self.caption = data.caption;

            if (data.audioDisplayFlag == 1) {
              self.soundBox = true;
              self.voiceSource = config.imageServerUrl + data.actAudio;
              self.ad_duration = data.ad_duration;
            }
  ...
  .chooseItem {
    ...
    .left {
      ...
      i {
        ...
        width: pTR(88);
        height: pTR(88);
        background: url(../../assets/czlImg/handicon.png) 0 0 no-repeat;
        background-size: 100%;
        margin-right: pTR(20);
      }
    }
    .right {
      ...
      i {
        ...
        width: pTR(88);
        height: pTR(88);
        background: url(../../assets/czlImg/consulticon.png) 0 0 no-repeat;
        background-size: 100%;
        margin-right: pTR(20);
      }
    }
...
.goline {
  width: pTR(656);
  height: pTR(126);
  background: url(../../assets/images/goLine.png) 0 0 no-repeat;
  background-size: 100%;
  margin: 0 auto pTR(25);
  position: relative;
  .gobtn {
    position: absolute;
    width: pTR(100);
    height: pTR(102);
    background: url(../../assets/images/goBtn.png) 0 0 no-repeat;
    background-size: 100%;
    right: pTR(30);
    top: 50%;
    margin-top: pTR(-50);
...
.saleMain {
  ...
  h2 {
    ...
    font-size: pTR(42);
    color: #0c0201;
    padding: pTR(65) 0 pTR(10);
  }
  p {
    text-align: center;
    color: #444444;
    font-size: pTR(28);
    padding: pTR(30) 0 pTR(40);
  }
  .saleMa {
    width: pTR(375);
    height: pTR(375);
  ...
  .sclose {
    width: pTR(60);
    height: pTR(60);
    background: url(../../assets/czlImg/sclose.png) 0 0 no-repeat;
    background-size: 100%;
    position: absolute;
    bottom: pTR(-120);
    left: 50%;
