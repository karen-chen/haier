  ...
  <div class="countdown_body">
    ...
    <div class="pop_mask" id="giftBox" v-show="isShowPrize">
      <div class="pop_box gift_box bounceIn">
        ...
        <div class="btn" @click="showQrcodeDlg">立即领取</div>
        <div class="off_btn" @click="closePrizeDlg">&times;</div>
      </div>
    </div>
 <!--中奖弹框 无图片-->
    <div class="pop_mask" id="giftBox" v-show="isShowPrize1">
    ...
    <!--红包名称-->
    <div class="pop_mask" v-show="hongbao">
      <!-- <div class="pop_mask"> -->
      <div class="pop_box hongbaoBox">
        <div class="personPic"><img :src="shopPic"></div>
        <div class="shopName">{{shoptitle}}</div>
        <div class="getmoney">{{prize_title}}</div>
        <div class="getBtn" @click="openmoneyBtn">马上领取</div>
        <div class="off_btn" @click="closePrizeDlg">&times;</div>
      </div>
    </div>
    <!--现金红包-->
    <div class="pop_mask" v-show="hongbao1">
      <!-- <div class="pop_mask"> -->
      <div class="pop_box hongbaoBox1">
        <div class="getmoney"><span>{{moneyNum}}</span>元</div>
        <div class="moneytips">红包将在24小时内发至你的微信, 请注意查收!</div>
        <div class="getBtn" @click="closePrizeDlg">我知道了</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    showQrcodeDlg() {
      ...
      if (this.isShowPrize1) {
        this.isShowPrize1 = false
      }
    },
    closeQrcodeDlg() {
      if (this.isShaking) {
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (data.ret == 0 && data.timerPrize) {
              ...
              _this.moneyNum = data.offerAmount / 100;
