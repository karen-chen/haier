<template>
  <div class="pcphone">
    ...
    <div class="startbox" v-show="startbox">
      ...
      <div class="startMa" v-show="startMa">
        <div class="picleft">
      ...
      <div class="waitBox" v-show="waitBox">
        <div class="cy1"></div>
        <div class="cy2"></div>
        <div class="cy3"></div>
        <div class="waitBtn"></div>
      </div>
  ...
  methods: {
    ...
    checkSpeedTimer() {
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data.ret == 0) {
            localStorage.setItem(_this.actSid + 'timerSid', data.timerSid);
          ...
          if (data.ret === "-2") {
            if (localStorage.getItem(_this.actSid + 'timerSid')) {
              console.log('lalalla');
              $.ajax({
                type: "GET",
                ...
                dataType: "jsonp",
                data: {
                  timerSid: localStorage.getItem(_this.actSid + 'timerSid'),
                },
                success: function(data) {
                  if (data.end == 1) {
                    ...
                    if (data.rank) {
                      ...
                      _this.rankList1length = data.rankLen;
                      _this.rankList1 = data.rank.slice(0, _this.rankList1length);
                    }
        ...
        success: function(data) {
          if (data.ret == 0) {
            if (data.rank) {
              ...
              _this.rankList1length = data.rankLen;
              _this.rankList1 = data.rank.slice(0, _this.rankList1length);
              console.log(_this.rankList1length);
...
.pcphone {
  ...
  .waitBox {
    ...
    .cy1 {
      ...
      width: pTR(528);
      height: pTR(528);
      background: url(../assets/czlImg/cyc3.png) 0 0 no-repeat;
      background-size: 100%;
      left: 50%;
      top: 50%;
      margin-left: pTR(-264);
      margin-top: pTR(-264);
      animation: cycbox 5s 1s infinite linear;
    }
    .cy2 {
      position: absolute;
      width: pTR(400);
      height: pTR(408);
      background: url(../assets/czlImg/cyc2.png) 0 0 no-repeat;
      background-size: 100%;
      left: 50%;
      top: 50%;
      margin-left: pTR(-200);
      margin-top: pTR(-200);
      animation: cycbox 10s 2s infinite linear;
    }
    .cy3 {
      position: absolute;
      width: pTR(238);
      height: pTR(238);
      background: url(../assets/czlImg/cyc1.png) 0 0 no-repeat;
      background-size: 100%;
      left: 50%;
      top: 50%;
      margin-left: pTR(-119);
      margin-top: pTR(-119);
      animation: cycbox 15s 3s infinite linear;
    }
    .waitBtn {
      position: absolute;
      width: pTR(862);
      height: pTR(220);
      background: url(../assets/czlImg/wait.png) 0 0 no-repeat;
      background-size: 100%;
      left: 50%;
      top: 50%;
      margin-left: pTR(-500);
      margin-top: pTR(-80);
      animation: opabox 5s infinite linear;
    }
  }
